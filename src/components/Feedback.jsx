...
function Feedback() {
  ...
  async function fetchallteachers() 
  {
    try 
    {
      const resp = await axios.get(`${process.env.REACT_APP_APIURL}/api/fetchallteacher`)
                ...
                <div className='col-lg-4 col-md-6 col-sm-12 mb-3'>
                    <div className="card" key={index}>
                      ...
                      <div className="card-content  text-center">
                        ...
                        {/* <button className="feedback-button" onClick={()=>next(item._id)}>Give Feedback</button> */}
                        {/* <Link to={"/givefeedback?id="+item._id}><button className="feedback-button">Give Feedback</button></Link> */}
                        <Link to={`/givefeedback?id=${item._id}`}><button className="feedback-button">Give Feedback</button></Link>

                        {/* <Link to={`/givefeedback1?id=${item._id}`}><button className="feedback-button">Give Feedback</button></Link> */}
