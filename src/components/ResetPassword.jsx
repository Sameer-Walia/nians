...
function ResetPassword()
{
    ...
    async function verifytoken()
    {
        try
        {
            const resp = await axios.get(`${process.env.REACT_APP_APIURL}/api/checktoken?token=` + token)
    ...
    async function resetpassword(e)
    {
        ...
        try
        {
            if (newpass === cnewpass)
            {
                ...
                const resp = await axios.put(`${process.env.REACT_APP_APIURL}/api/resetpassword`, apidata, { withCredentials: true })
                if (resp.data.statuscode === 1)
                {
